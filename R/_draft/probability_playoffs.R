...
sim11 <- readRDS("./data/simulation_v5_week11_final.rds")
...
team_odds %>% 
  mutate( imageUrl = if_else(Week==12, imageUrl, as.character(NA))) %>% 
  ...
  geom_text_repel(aes(label=if_else(Week==12, team, ""), color=team), direction="y", nudge_x = 1, hjust="left") +
