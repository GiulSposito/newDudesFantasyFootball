...
plotDFAProjections <- function(.proj, .pos, .slice=32) {
  proj_data <- .proj %>% 
    ...
    top_n(.slice, estimate) %>% 
    mutate(pos_rank=1:nrow(.))
  
  nu_y <- max(proj_data$conf.high, na.rm = T) / 9
  
  proj_data %>% 
    group_by( id ) %>% 
    mutate( r_pos = max(conf.high, points) ) %>% # posicao do label
    ungroup() %>% 
    ggplot(aes(team=team)) +
    geom_pointrange(aes(x=reorder(pos_rank,desc(pos_rank)),
                        y=estimate, 
...
plotDFAProjections(projections, "QB", 14+14+5)
...
plotDFAProjections(projections, "WR",(5*16))
...
plotDFAProjections(projections, "RB", (5*16))
...
plotDFAProjections(projections, "TE",(3*14))
...
plotDFAProjections(projections, "K",(2*14))
...
plotDFAProjections(projections, "DST",32)
