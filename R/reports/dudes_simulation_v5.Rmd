...
projections <- sim$proj_table %>% 
  ...
  select(id, projPts = estimate, pos_rank, drop_off, sd_pts, floor=conf.low, ceiling=conf.high, tier) 
...
rosters <- sim$teams %>% 
  ...
  inner_join(readRDS(glue("./data/dudesffa_projpoints_week{.week}.rds")), by="id") %>% 
  mutate(
    conf.low  = if_else(injuryAbbr %in% c("IR","IA","Out","CVD"),0,conf.low),
    estimate  = if_else(injuryAbbr %in% c("IR","IA","Out","CVD"),0,estimate),
    conf.high = if_else(injuryAbbr %in% c("IR","IA","Out","CVD"),0,conf.high)
  )
